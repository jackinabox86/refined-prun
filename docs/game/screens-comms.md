...
- Message lists name only an opaque `channelId`; they don't identify the requested command or group. Never treat the first list seen in a WebSocket session as a particular channel: restored comms buffers can emit unrelated lists first. Arm collection when opening the requested channel, and use matching `CHANNEL_CLIENT_MEMBERSHIP` data when the server supplies it.
