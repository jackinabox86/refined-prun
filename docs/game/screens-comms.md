...
- The connection-level `CHANNEL_CHANNEL_LIST` push identifies every joined channel as `{type, identifier, channelId, ...}`. Use its `identifier` mapping to establish a private group's opaque `channelId` before routing message-list pushes; the mapping is replayed after reconnects.
...
- Message lists name only an opaque `channelId`; they don't identify the requested command or group. Never treat the first list seen in a WebSocket session—or while a channel command is in flight—as a particular channel: restored comms buffers can emit unrelated lists first. Accept history only after `CHANNEL_CHANNEL_LIST` or `CHANNEL_CLIENT_MEMBERSHIP` maps the requested identifier to that exact id. Bound callers waiting for that safe ordering and fail closed if it never arrives; a dropped one-shot history push must not leave the UI or an action workflow waiting forever.
