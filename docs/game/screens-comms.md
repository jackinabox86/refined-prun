...
- Message lists name only an opaque `channelId`; they don't identify the requested command or group. For a named private group, correlate its `CHANNEL_CLIENT_MEMBERSHIP.identifier` to the channel id before consuming history. Restored comms buffers can emit unrelated lists first, and the target list can arrive before or after membership, so retain candidate lists by channel id instead of treating the first list as the target.
