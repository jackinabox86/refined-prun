...
even once the URL itself is correct. Check `.gitattributes` before committing the asset:
`*.png`/`*.jpg`/`*.svg` are Git LFS-tracked, but `*.gif` is not, so a gif lands in Git
history as a raw blob.
